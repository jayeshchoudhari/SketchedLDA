...
docWordMat = np.zeros((numDocuments, vocabsize))

inputFile = sys.argv[1]

sketchMat = np.zeros((sketchSize, numDocuments))

sketchPossVals = [2/(2*sketchSize)**(1/3), -1/(2*sketchSize)**(1/3)]
sketchValDist = [0.33333, 0.66667]


for i in range(sketchSize):
	for j in range(numDocuments):
		possVal = np.random.choice(sketchPossVals, 1, True, sketchValDist)[0]
		sketchMat[i,j] = possVal
...
for line in f1:
	...
	for j in range(len(flds)):
		docWordMat[docId, j] = int(flds[j])

	docId += 1


sketchedDocs = np.matmul(sketchMat, docWordMat)

# alpha, beta = spectral_lda(docWordMat, alpha0=1, k=numTopics, l1_simplex_proj=True)
alpha, beta = spectral_lda(sketchedDocs, alpha0=1, k=numTopics, l1_simplex_proj=True)
